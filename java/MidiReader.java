	...
	public static void main(String[] args) throws Exception {
		...
		while (true) { // loops until working filename is entered
			...
			try {
				...
				System.out.println("ファイルの読み込みエラー: " + filename + ": " + exception);
				continue;
			}
			break;
		}

		timestampConverter = ((sequence.getMicrosecondLength() / 1000.0) / sequence.getTickLength()); // conversion rate
																										// for MIDI
																										// ticks to
																										// milliseconds
		...
		for (Track track : sequence.getTracks()) { // looping through tracks (1 track to 1 buzzer)
			...
			System.out.print(trackNumber + "番のトラックをどこのピンでながす？"); // getting buzzer pin
			...
			for (int i = 0; i < track.size(); i++) { // iterating through notes to get timestamps
				...
				if (message instanceof ShortMessage) { // making sure message is valid message
					...
					writer.print(((long) (timestamp * timestampConverter)) + ","); // converting to milliseconds and
																					// writing to melody.h
			...
			for (int i = 0; i < track.size(); i++) { // iterating through notes to get frequencies
				...
				if (message instanceof ShortMessage) { // making sure message is valid message
					...
					if (sm.getCommand() == NOTE_OFF) { // whether event is the end of note...
						...
						frequency = (int) ((Math.pow(2.0, (sm.getData1() - 69.0) / 12.0)) * 440.0); // converts key # to
																									// frequency (in
																									// Hertz)
					}
		...
		scan.close();
		writer.close();
		System.out.println("melody.hへの書き込みが完了しました！");
